...
For status details, e.g. information about EXCEPTIONAL_SITUATION, see xref:facility-details[`statusDescription`].
...
Below is an example of details of a facility.
...
If available, the the latest known number of available parking places is reported for each facility, capacity type and usage combination.
...
xref:authentication[Authenticated] operators may update the utilization for their facilities.

include::{generated}/utilization-update-example/http-request.adoc[]

==== Prediction

Utilization predictions are available for all facilities which have utilization data.
The predictions are updated at regular intervals and they are available for up to 24 hours into the future.
It is possible to query predictions using relative time (`after` parameter in `hhmm` format)
or absolute time (`at` parameter in ISO 8601 date time format).

NOTE: As of May 2015, the actual prediction algorithms have not yet been implemented,
so the predictions are not yet useful, but it's possible to start implementing applications which use the API.

include::{generated}/prediction-relative-example/http-request.adoc[]
...
From a hub on may continue to all places reachable via public transportation.
...
Basic information of those facilities may be searched from `facilities` resource using `ids`-parameter,
