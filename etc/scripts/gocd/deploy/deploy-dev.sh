...
function start() {
  ssh $SSH_OPTS $AWS_TEST "cd $DST_ENV; nohup java -Xms1G -Xmx1G  -Djava.awt.headless=true -jar $DST_JAR --spring.profiles.active=env_$DST_ENV --server.port=$PORT > app.out &"
}
